...
<div class="form-group col-sm-12 col-md-12"> 
          	<div class="container">
							...
							<div class="container">
								<input type="checkbox" name="checkAll" id="location_head" class="permission"/></label>
...
$(".permission").click(function (e) {
        ...
        if(id="location_head")
        {
            var location_head = ["state_list","district_list"];
            for(i=0;i<location_head.length;i++)
            {
            var id_new = "all_" + location_head[i] + "_master";
            var id_newest = location_head[i];
            if ($(location_head[i]).hasClass(id_new) == true)
            {
                if ($(location_head[i]).prop('checked') != true) {
            ...
            {
                var classname = "." + $(location_head[i]).attr("class").split(' ')[0];
                var all_classname = ".all_" + $(location_head[i]).attr("class").split(' ')[0]+"_master";
                var counter = 0;
                $(classname).each(function () {
                    if ($(location_head[i]).prop('checked') != true && $(all_classname).attr("class") != $(location_head[i]).attr("class")) {
                        counter++;
                    }
                });
                if (counter > 0)
