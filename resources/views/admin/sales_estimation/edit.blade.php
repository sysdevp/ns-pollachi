...
<div class="col-12 body-sec">
  <div class="card container-fluid px-0">
    ...
    <div class="card-header px-2">
      <div class="row">
        <div class="col-4">
          <h3>Edit Sale Estimation</h3>
        </div>
        <div class="col-8 mr-auto">
          <ul class="h-right-btn mb-0 pl-0">
            <li><button type="button" class="btn btn-success"><a href="{{ url('sales_estimation/index/0') }}">Back</a></button></li>
          </ul>
        </div>
      </div>
    </div>
...
<form  method="post" class="form-horizontal" action="{{ route('sales_estimation.update',$sale_estimation->sale_estimation_no) }}" id="dataInput" enctype="multipart/form-data">
      {{csrf_field()}}
      @method('PATCH')
                       ...
                       <div class="row col-md-10">
                                ...
                                <div class="col-md-2">
                                  ...
                                  <div class="">
                                    <input type="hidden" readonly="" id="voucher_no" name="voucher_no" value="{{ $sale_estimation->sale_estimation_no }}">
                                    <input type="hidden" id="voucher_number" name="voucher_number" value="{{ $sale_estimation->voucher_no }}">
                                    <font size="2">{{ $sale_estimation->sale_estimation_no }}</font>
                                  </div>
                                ...
                                <input type="date" class="form-control voucher_date  required_for_proof_valid" id="voucher_date" placeholder="Voucher Date" name="voucher_date" value="{{ $sale_estimation->sale_estimation_date }}" <?php echo Mandatoryfields::validation('salesestimation_voucherdate');?> autofocus>
                     ...
                     <div class="col-sm-8">
                      <select class="js-example-basic-multiple col-12 form-control custom-select customer_id" onchange="customer_details()" name="customer_id" id="customer_id" <?php echo Mandatoryfields::validation('salesestimation_customerid');?>>
                        @if(isset($sale_estimation->customer->name) && !empty($sale_estimation->customer->name))
                           <option value="{{ $sale_estimation->customer->id }}">{{ $sale_estimation->customer->name }}</option>
                           @else
                  ...
                  <div class="form-group row">
                     <div class="col-sm-8">
                      <select class="js-example-basic-multiple col-12 form-control custom-select salesmen_id" name="salesmen_id" id="salesmen_id" <?php echo Mandatoryfields::validation('salesestimation_salesmenid');?>>
                           ...
                           <option value="{{ $sale_estimation->salesman->id }}">{{ $sale_estimation->salesman->name }}</option>
                           @else
                  ...
                  <div class="form-group row">
                     <div class="col-sm-8">
                      <select class="js-example-basic-multiple col-12 form-control custom-select agent_id" name="agent_id" id="agent_id" <?php echo Mandatoryfields::validation('salesestimation_agentid');?>>
                           @if(isset($sale_estimation->agent->name) && !empty($sale_estimation->agent->name))
                           <option value="{{ $sale_estimation->agent->id }}">{{ $sale_estimation->agent->name }}</option>
                           @else
...
<div class="form-row">
              ...
              <div class="col-md-12" id="middlecol">
                ...
                <table class="table table-responsive" id="team-list">
                  ...
                  <input type="hidden" name="total_price" value="{{$sale_estimation->total_net_value}}" id="total_price">
                  ...
                  <tfoot>
                    <tr>
                      ...
                      <input type="number" class="form-control overall_discount" id="overall_discount" name="overall_discount" oninput="overall_discounts()" pattern="[0-9]{0,100}" title="Numbers Only" value="{{ $sale_estimation->overall_discount }}" <?php echo Mandatoryfields::validation('salesestimation_overall_discount');?>>
                      </div>
                    </div>
                  ...
                  <div class="form-group row">
                    ...
                    @endif
                       ...
                       <div class="col-md-12" style="float: right;">
                        ...
                        <font color="black" style="font-size: 150%; margin-left: 700px; font-weight: 900;">NET Value :</font>&nbsp;<font class="total_net_value" style="font-size: 150%; font-weight: 900;">{{$sale_estimation->total_net_value}}</font> 
                       </div>
                    <!-- net value -->
                      ...
                      <input type="text" class="form-control round_off" readonly="" value="{{ $sale_estimation->round_off }}" id="round_off" name="round_off" >
                      </div>
...
$(document).on("click",".remove_items",function(){
    ...
    $.ajax({
           type: "GET",
            url: "{{ url('sales_estimation/remove_data/{id}') }}",
...
$(document).on("click",".edit_items",function(){
  ...
  var actual_discount_percent = $('#actual_discount_percent'+id).val();
  var actual_discount_rs = $('#actual_discount_rs'+id).val();
  ...
  var disc_value = parseFloat(discount_val)/parseFloat(quantity);
   $('.discount_rs').val(actual_discount_rs);
   $('.discount_percentage').val(actual_discount_percent);
   // discount_calc();
 ...
 else
 {
  ...
  $('.last_purchase'+td_id).text($('#last_purchase_rate').val());
   ...
   if($('.discount_percentage').val() == '' && $('.discount_rs').val() == '')
   {
    ...
    $('#actual_discount_percent'+td_id).val(discount);
    $('#actual_discount_rs'+td_id).val(discount);
    var q=calculate_total_discount();
    $('#total_discount').val(q.toFixed(2));
    $('#disc_total').val(q.toFixed(2));
   ...
   else
   {
    ...
    $('#actual_discount_percent'+td_id).val($('.discount_percentage').val());
    $('#actual_discount_rs'+td_id).val($('#discount').val());
    var q=calculate_total_discount();
    $('#total_discount').val(q.toFixed(2));
    $('#disc_total').val(q.toFixed(2));
   }
...
if(append_value == 1)
{
  var row_id=$('#last').val();
      ...
      $.ajax({  
        ...
        type: "GET",
        url: "{{ url('sales_estimation/getdata/{id}') }}",
...
// return false;
      ...
      $.ajax({
           type: "POST",
            url: "{{ url('sales_estimation/last_purchase_rate/') }}",
            data: { id: item_code },
           success: function(data) {
...
function item_with_same_data(item_code)
{
  $.ajax({
        ...
        type: "GET",
        url: "{{ url('sales_estimation/same_items/{id}') }}",
        data: { id: item_code },
...
function browse_item()
{
  ...
  $.ajax({  
        ...
        type: "GET",
        url: "{{ url('sales_estimation/browse_item/{id}') }}",
...
function categories_check()
{
  ...
  $.ajax({  
        ...
        type: "GET",
        url: "{{ url('sales_estimation/change_items/{id}') }}",
...
function brand_check()
{
  ...
  $.ajax({
        ...
        type: "POST",
        url: "{{ url('sales_estimation/brand_filter/') }}",
        data: {brand: brand },             
...
function customer_details()
{
  ...
  $.ajax({
           type: "POST",
            url: "{{ url('sales_estimation/address_details/') }}",
