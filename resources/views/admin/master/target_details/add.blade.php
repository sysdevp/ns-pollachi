...
<div class="col-12 body-sec">
  <div class="card container px-0">
    ...
    <div class="card-body">
      ...
      <form  method="post" class="form-horizontal needs-validation" novalidate action="{{route('salesman-target-details.store')}}" enctype="multipart/form-data">
      {{csrf_field()}}
        ...
        <tbody>
          ...
          @foreach($locations as $key => $value)
            <tr>
              <td>{{ $key+1 }}</td>
              <td>{{ $value->name}}<input type="hidden" name="location_id[]" value="{{ $value->id}}"></td>
              <td><input type="number" placeholder="Target Amount" value="0" name="target_amount[]"></td>
              <td><input type="date" autocomplete="off" name="target_from_date[]"></td>
              <td><input type="date" autocomplete="off" name="target_date[]"></td>
            </tr>
         @endforeach
        </tbody>
      </table>
