          ...
          @foreach($sale_entry as $key => $value)
            <tr>
              ...
              <td> 
                ...
                <a href="{{ route('sales_entry.edit',$value->s_no) }}" class="px-1 py-0  text-white rounded" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a>
          ...
          @foreach($sale_entry_beta as $key => $value)
            <tr>
              ...
              <td> 
                @if($value->cancel_status == 0)
                <a href="{{ route('sales_entry.show',$value->s_no) }}" class="px-1 py-0 text-white rounded" title="View">><i class="fa fa-eye" aria-hidden="true"></i></a>
                <a href="{{ url('sales_entry/edit_beta/'.$value->s_no) }}" class="px-1 py-0  text-white rounded" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a>
                <a href="{{url('sales_entry/delete/'.$value->s_no )}}" onclick="return confirm('Are you sure ?')" class="px-1 py-0  text-white rounded" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></a>

                <a href="{{ url('sales_entry/cancel_beta/'.$value->s_no) }}" class="px-2 py-1 bg-warning text-white rounded">Cancel</a>

                <br><br>
                <a href="{{url('sales_entry/item_beta_details/'.$value->s_no )}}" class="px-1 py-0 bg-info text-white rounded"><i class="fa fa-eye" aria-hidden="true"></i>Item Details</a>
                <a href="{{url('sales_entry/expense_beta_details/'.$value->s_no )}}" class="px-1 py-0 bg-info text-white rounded"><i class="fa fa-eye" aria-hidden="true"></i>Expense Details</a>
                @else
