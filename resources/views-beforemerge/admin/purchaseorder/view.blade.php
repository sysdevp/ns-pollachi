        ...
        <tbody id="test1">
          @foreach($purchaseorder as $key => $value)
            <tr>
              ...
              <td> 
                @if($value->status == 0)
                <a href="{{ route('purchase_order.show',$value->po_no) }}" class="px-1 py-0 text-white rounded" title="View">><i class="fa fa-eye" aria-hidden="true"></i></a>
                <a href="{{ route('purchase_order.edit',$value->po_no) }}" class="px-1 py-0  text-white rounded" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a>
                <a href="{{url('purchase_order/delete/'.$value->po_no )}}" onclick="return confirm('Are you sure ?')" class="px-1 py-0  text-white rounded" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></a>
        ...
        <tbody id="test2" style="display: none;">
          @foreach($purchaseorderbeta as $key => $value)
            <tr>
              ...
              <td> 
                @if($value->status == 0)
                <a href="{{ url('purchase_order/show_beta/'.$value->po_no) }}" class="px-1 py-0 text-white rounded" title="View">><i class="fa fa-eye" aria-hidden="true"></i></a>
                ...
                <a href="{{url('purchase_order/delete_beta/'.$value->po_no )}}" onclick="return confirm('Are you sure ?')" class="px-1 py-0  text-white rounded" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></a>

                <a href="{{ url('purchase_order/cancel_beta/'.$value->po_no) }}" class="px-2 py-1 bg-warning text-white rounded">Cancel</a>

                <br><br>
                <a href="{{url('purchase_order/item_beta_details/'.$value->po_no )}}" class="px-1 py-0 bg-info text-white rounded"><i class="fa fa-eye" aria-hidden="true"></i>Item Details</a>
                <a href="{{url('purchase_order/expense_beta_details/'.$value->po_no )}}" class="px-1 py-0 bg-info text-white rounded"><i class="fa fa-eye" aria-hidden="true"></i>Expense Details</a>
                @else
